...
public class TestUnit {
    ...
    public static void main(String[] args) {
        try {
        	...
        	String mergePath = IPUtil.class.getResource("/ip.merge.txt").getPath();
        	String errorPath = IPUtil.class.getResource("/error_log.txt").getPath();
      
            DbSearcher _searcher = new DbSearcher(new DbConfig(), dbPath);
            BufferedReader bfr   = new BufferedReader(new FileReader(mergePath));
            BufferedWriter bwr   = new BufferedWriter(new FileWriter( errorPath, true));
            int errCount  = 0;
            int lineCount = 0;
            String str = null;
